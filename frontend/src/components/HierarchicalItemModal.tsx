...
export default function HierarchicalItemModal({ onClose, onSuccess, mode, type, projectData, parentData, initialData }: HierarchicalItemModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      if (!initialData) throw new Error('Item inválido');
  ...
  const typeLabels: Record<ItemType | 'BUG', string> = { EPIC: 'Épico', STORY: 'História', TASK: 'Atividade', SUBTASK: 'Subtask', BUG: 'Bug' };
            ...
            <>
              ...
              {type === 'STORY' && (
                <div>
                  ...
                  <textarea className="input-glass" placeholder="Regras para aprovação desta história..." rows={3} value={formData.acceptance_criteria} onChange={e => handleChange('acceptance_criteria', e.target.value)} style={{ width: '100%', padding: '10px 15px' }} />
                </div>
              )}
